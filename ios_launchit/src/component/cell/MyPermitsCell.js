...
class MyPermitsCell extends Component {
    ...
    render() {
        ...
        return (
            <TouchableOpacity style = {[styles.container, style]} onPress = {(item) => this.props.onPress(item)}>
                ...
                <Text style = {[styles.text, {...fonts.H2_Regular, color: colors.PRIMARY, marginRight: 30,width:'60%'}]}>{item.folderNumber}</Text>
                 <Text style = {[styles.text,{...fonts.H3_Bold}]}>{item.folderTypeDesc}</Text>
                <View style={styles.locationStyle}>
                <Image 
                    resizeMode = 'contain'
                    source={images.location}
                    style = {styles.imageStyle}/>
                <Text style = {[styles.text, {color: colors.TEXT_BROWN_COLOR,marginTop:0,marginLeft:8,width:'80%'}]}>{item.folderName}</Text>
                </View>
            </TouchableOpacity>
        )
    }
    ...
    _getStatus() {
        switch(this.props.item.statusDesc) {
            case "Active" : return "Active"
            case "Pending" : return "Pending"
            case "Issued" : return "Issued"
            default : return this.props.item.statusDesc
        }
    }
